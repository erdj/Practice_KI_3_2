...
export const ConnectRoom = ({ closeHandler, socket }: ConnectRoomProps) => {
  ...
  return (
    <GameContainer className={styles.connectRoom}>
      ...
      <p onClick={() => randomRoom()}>RANDOM GAME</p>
  ...
  function randomRoom() {
    if (roomsState.rooms) {
      // console.log(Math.random() * roomsState.rooms.length);

      connectRoom(roomsState.rooms[Math.floor(Math.random() * roomsState.rooms.length)]);
    }
  }
};
