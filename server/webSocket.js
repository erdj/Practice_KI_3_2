      ...
      if (games[game.roomId].result.creator !== '' && games[game.roomId].result.guest !== '') {
        ...
        if (isNoTurn) {
          broadcastMessageById(
            {
              event: 'noTurn',
              payload: { message: 'someone didnt choose a turn', game },
            },
            game.creatorId,
          );
          broadcastMessageById(
          ...
          await axios.delete('http://localhost:5000/api/games/' + game._id, DB_REQ_CONFIG);
          console.log(game);
          await axios.put(
            'http://localhost:5000/api/users/' + game.creatorId,
            { gameId: '' },
            DB_REQ_CONFIG,
          );
          await axios.put(
            'http://localhost:5000/api/users/' + game.guestId,
            { gameId: '' },
            DB_REQ_CONFIG,
          );
